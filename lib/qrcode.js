...
/**
 * Gera PDF para uma família com QR code e lista de membros
 * @param {Object} family - { familyName, qrCodeIMG, id, members }
 * @returns {Promise<Buffer>}
 */
async function generateFamilyPDF(family) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Header
      doc.fontSize(24).font("Helvetica-Bold").text("TáNaLista", { align: "center" });
      doc.moveDown(0.5);
      doc.fontSize(12).font("Helvetica").fillColor("#666666").text("Convite de Família", { align: "center" });
      doc.moveDown(1);
      doc.moveTo(50, doc.y).lineTo(545, doc.y).strokeColor("#e5e7eb").lineWidth(1).stroke();
      doc.moveDown(1);

      // Nome da família
      doc.fontSize(18).font("Helvetica-Bold").fillColor("#111111").text(family.familyName, { align: "center" });
      doc.moveDown(1.5);

      // QR Code
      let qrBuffer;
      if (family.qrCodeIMG && family.qrCodeIMG.startsWith("data:image/png;base64,")) {
        const base64Data = family.qrCodeIMG.replace("data:image/png;base64,", "");
        qrBuffer = Buffer.from(base64Data, "base64");
      } else {
        qrBuffer = await generateQRCodeBuffer(family.id);
      }

      const pageWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right;
      const imgSize = 200;
      const imgX = doc.page.margins.left + (pageWidth - imgSize) / 2;
      doc.image(qrBuffer, imgX, doc.y, { width: imgSize, height: imgSize });
      doc.moveDown(imgSize / 12 + 1);

      doc.fontSize(9).font("Helvetica").fillColor("#9ca3af").text(`ID: ${family.id}`, { align: "center" });
      doc.moveDown(1.5);

      // Membros
      if (family.members && family.members.length > 0) {
        doc.fontSize(12).font("Helvetica-Bold").fillColor("#111111").text("Membros da família:", { align: "left" });
        doc.moveDown(0.5);
        family.members.forEach((m) => {
          doc.fontSize(11).font("Helvetica").fillColor("#374151").text(`• ${m.nome}`, { indent: 20 });
        });
        doc.moveDown(1);
      }

      doc.fontSize(10).fillColor("#6b7280").text("Apresente este QR code na entrada do evento.", { align: "center" });
      doc.end();
    } catch (err) {
      reject(new Error(`Erro ao gerar PDF da família: ${err.message}`));
    }
  });
}
